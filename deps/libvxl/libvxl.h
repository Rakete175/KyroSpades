...
static inline uint32_t libvxl_copy_chunk_get_color(
	struct libvxl_chunk_copy* copy, size_t x, size_t y, size_t z) {
	size_t lx = x - copy->grid_x, ly = y - copy->grid_y;
	if(lx >= LIBVXL_CHUNK_SIZE || ly >= LIBVXL_CHUNK_SIZE || z >= copy->depth)
		return 0;
	return copy->colors[z + (lx + ly * LIBVXL_CHUNK_SIZE) * copy->depth];
}

static inline bool libvxl_copy_chunk_is_solid(struct libvxl_chunk_copy* copy,
											  size_t x, size_t y, size_t z) {
	if(x >= copy->width || y >= copy->height || z >= copy->depth)
		return true;

	size_t lx = (x + copy->width - copy->geo_x) % copy->width;
	size_t ly = (y + copy->height - copy->geo_y) % copy->height;
	if(lx >= copy->geo_w || ly >= copy->geo_h)
		return true;

	size_t offset = z + (lx + ly * copy->geo_w) * copy->depth;
