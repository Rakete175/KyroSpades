...
void player_save_corpse(int player_id) {
        if(player_id < 0 || player_id >= PLAYERS_MAX)
                return;
        /* Performance Mode: never keep corpses around — each dead body is
           a kv6_render + kv6_calclight call every frame. */
        if(!settings.disable_corpse_despawn || settings.performance_mode)
                return;
