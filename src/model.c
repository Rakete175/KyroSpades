...
void kv6_calclight(int x, int y, int z) {
        /* Performance Mode: skip the per-entity map_sunblock raycast (walks
           up to ~5 voxels above the entity every call). Force full-bright
           ambient/diffuse so models are still lit, just uniformly. The
           glLightfv calls are cheap and stay so models don't go black. */
        float f = 1.0F;

        if(!settings.performance_mode && x >= 0 && y >= 0 && z >= 0)
